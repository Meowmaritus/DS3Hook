    ...
    static internal class Kernel
    {
        ...
        internal static bool CheckAddress(IntPtr addr, uint length, string action)
        {
            ...
            else
            {
                    ...
                    var query = VirtualQueryEx(Hook.DarkSouls3Handle.GetHandle(), (IntPtr)addr, out var info, (uint)Marshal.SizeOf(typeof(MEMORY_BASIC_INFORMATION)));
                    if (query == 0)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            if (VirtualProtectEx(Hook.DarkSouls3Handle.GetHandle(), (IntPtr)addr, length, PAGE_EXECUTE_READWRITE, out var garbage))
                            {

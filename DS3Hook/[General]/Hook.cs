...
namespace DS3Hook
{
    public static class Hook
    {
        ...
        public static SafeDarkSouls3Handle DarkSouls3Handle { get; private set; } = new SafeDarkSouls3Handle();
        ...
        public static string RAsciiStr(Memloc addr, int maxLength)
        {
            ...
            if (maxLength != 0)
            {
                ...
                {
                    Kernel.ReadProcessMemory_SAFE(DarkSouls3Handle.GetHandle(), IntPtr.Add(addr, loc), bytes, 1, IntPtr.Zero);
        ...
        {
            Kernel.WriteProcessMemory_SAFE(DarkSouls3Handle.GetHandle(), addr, BitConverter.GetBytes(val), 1, IntPtr.Zero);
        }


        public static void WInt16(Memloc addr, Int16 val)
        {
            Kernel.WriteProcessMemory_SAFE(DarkSouls3Handle.GetHandle(), addr, BitConverter.GetBytes(val), 2, IntPtr.Zero);
        }
        ...
        {
            Kernel.WriteProcessMemory_SAFE(DarkSouls3Handle.GetHandle(), addr, BitConverter.GetBytes(val), 8, IntPtr.Zero);
        }
        ...
        {
            Kernel.WriteProcessMemory_SAFE(DarkSouls3Handle.GetHandle(), addr, System.Text.Encoding.ASCII.GetBytes(str).Concat(new byte[] { 0 }).ToArray(), str.Length + 1, IntPtr.Zero);
        }
